...
def main(
    ...
    model_template_dir: Optional[Path] = typer.Option(
        None, "--model-template-dir"
    ),
...
def generate_code(
    ...
    environment: Environment = Environment(
        loader=FileSystemLoader(
            (
                template_dir
                if template_dir
                else f"{Path(__file__).parent}/template"
            ),
            encoding="utf8",
        ),
    )
    ...
    visitors_path = [
        *builtin_visitors,
        *(custom_visitors if custom_visitors else []),
    ]
    for visitor_path in visitors_path:
        ...
        if hasattr(module, "visit"):
            ...
            raise Exception(
                f"{visitor_path.stem} does not have any visit function"
            )
                ...
                with open(Path(output_dir.joinpath("main.py")), 'r') as file:
                    ...
                    if "app.include_router" in content:
                        tags = sorted(
                            set(
                                tag.strip()
                                for tag in str(specify_tags).split(",")
                            )
                        )
...
#   filename:  {Path(input_name).name}"""
    ...
    for path, code in results.items():
        ...
        if parent_path not in visited_init_paths:
            visited_init_paths.add(parent_path)
            init_path = Path(parent_path, '__init__.py')
            with output_dir.joinpath(init_path).open(
                "wt", encoding=encoding
            ) as file:
                pass
